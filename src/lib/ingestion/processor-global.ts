...
export function processGlobalRow(raw: RawRow, uploadId: string, sourceRow: number) {
  const mapped = {
    ...
    service_number: str(raw['Número serviço'] ?? raw['Numero Serviço'] ?? raw['Número Serviço']),
    ...
    acceptance_date: parseGlobalDate(raw['Data Aceitação de Sinistro'] ?? raw['Data Aceitação Sinistro']),
    manager: str(raw['Gestor']),
    responsibility_status: str(raw['Estado Responsabilidade']),
    tempestade_leslie: str(raw['Tempestade Leslie']),
    fraud_suspicion: str(raw['Suspeita de Fraude'] ?? raw['Supeita de Fraude '] ?? raw['Supeita de Fraude']),
    co_insurance: str(raw['Co-Seguro'] ?? raw['Co Seguro']),
    ...
    expertise_costs: parseNumeric(raw['Despesas c/Peritagem'] ?? raw['Despesas c Peritagem']),
    general_expenses: parseNumeric(raw['Despesas Gerais']),
    service_creation_date: parseGlobalDate(raw['Data da Criação do Serviço']),
    service_booking_date: parseGlobalDate(raw['Data de agendamento do Serviço'] ?? raw['Data da Marcação do Serviço']),
    first_responsibility_date: parseGlobalDate(raw['Primeira data de definição de responsabilidade']),
    first_payment_date: parseGlobalDate(raw['Primeira Data do Pagamento da Indemnização/Reparação'] ?? raw['Primeira Data do Pagamento da Indemnização Reparação']),
    last_payment_date: parseGlobalDate(raw['Última Data do Pagamento da Indemnização/Reparação'] ?? raw['Última Data do Pagamento da Indemnização Reparação']),
